...
import { 
  ...
  Upload, Copy, Calendar, Award, FolderTree 
} from 'lucide-react';
...
function AssignmentModal({ modalData, onSave, onClose }) {
  const [formData, setFormData] = useState(modalData.existingBlock || {
    ...
    submissionType: 'text',
    rubric: [],
    // New code checking fields
    codeCheckingEnabled: false,
    codeChecking: {
      language: 'javascript',
      repoName: '',
      requiredFiles: [], // Changed from single fileName to array of files
      mainFile: '', // Which file to run for testing
      expectedOutput: '',
      testCases: []
    }
  });
  ...
  // Test case functions
  const addTestCase = () => {
    const newTestCases = [...(formData.codeChecking.testCases || []), { input: '', output: '' }];
    setFormData({
      ...formData,
      codeChecking: { ...formData.codeChecking, testCases: newTestCases }
    });
  };

  const removeTestCase = (index) => {
    const newTestCases = formData.codeChecking.testCases.filter((_, i) => i !== index);
    setFormData({
      ...formData,
      codeChecking: { ...formData.codeChecking, testCases: newTestCases }
    });
  ...
  const updateTestCase = (index, field, value) => {
    const newTestCases = [...formData.codeChecking.testCases];
  ...
  const addRequiredFile = () => {
    const newFiles = [...(formData.codeChecking.requiredFiles || []), { path: '', description: '' }];
    setFormData({
      ...formData,
  ...
  const updateRequiredFile = (index, field, value) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          {/* CODE CHECKING SECTION */}
          <div className="border-t border-gray-200 pt-4">
            <div className="flex items-center gap-2 mb-4">
              <input
                type="checkbox"
                id="codeCheckingEnabled"
                checked={formData.codeCheckingEnabled}
                onChange={(e) => setFormData({ ...formData, codeCheckingEnabled: e.target.checked })}
                className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
              />
              <label htmlFor="codeCheckingEnabled" className="flex items-center gap-2 text-sm font-medium text-gray-700">
                <Code className="w-4 h-4" />
                Enable Automated Code Checking
              </label>
            </div>
            ...
            {formData.codeCheckingEnabled && (
              <div className="bg-indigo-50 rounded-lg p-4 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Main File to Execute</label>
                    <input
                      type="text"
                      ...
                      className="w-full border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500"
                      placeholder="e.g., index.js or src/main.py"
                    />
                    <p className="text-xs text-gray-500 mt-1">Which file to run for testing</p>
                  </div>
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Required GitHub Repo Name</label>
                  <input
                    type="text"
                    ...
                    className="w-full border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500"
                    placeholder="e.g., assignment-hello-world"
                  />
                  <p className="text-xs text-gray-500 mt-1">Students must create a repo with this exact name</p>
                </div>
                ...
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <label className="block text-sm font-medium text-gray-700">Required Files & Structure</label>
                  </div>
                  <p className="text-xs text-gray-500 mb-3">Specify all files that must exist in the repo (supports folders)</p>
                  ...
                  {formData.codeChecking.requiredFiles?.map((file, index) => (
                    <div key={index} className="flex gap-2 mb-2">
                      <input
                        type="text"
                        value={file.path}
                        onChange={(e) => updateRequiredFile(index, 'path', e.target.value)}
                        className="flex-1 border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 text-sm"
                        placeholder="e.g., src/index.js or components/App.jsx"
                      />
                      <input
                        type="text"
                  ...
                  {formData.codeChecking.requiredFiles?.length > 0 && (
                    <div className="mt-3 bg-white rounded p-3 border border-indigo-200">
                      <p className="text-xs font-medium text-gray-700 mb-1">Preview of required structure:</p>
                      <div className="font-mono text-xs text-gray-600 space-y-1">
                        {formData.codeChecking.requiredFiles.map((file, i) => (
                ...
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">What the code should print/output when run</p>
                </div>

                {/* Optional: Test Cases */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Test Cases (Optional)</label>
                  <p className="text-xs text-gray-500 mb-2">For testing with different inputs</p>
